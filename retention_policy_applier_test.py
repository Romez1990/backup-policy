...
def backups():
    ...
    return [
        ...
        for name, _ in backup_names
    ]


def test_retention_policy_applier(backups: list[BackupFile]) -> None:
    ...
    applier = RetentionPolicyApplier(logger=mock_logger, retention_policies=retention_policies)
    
    applier.apply(backups)

    result = [(backup.file_path.name, not backup.should_delete) for backup in backups]
    
    backups_to_delete = [backup for backup in backups if backup.should_delete]
    backups_to_keep = [backup for backup in backups if not backup.should_delete]
    ...
    assert len(backups_to_keep) < len(backups), "Expected to delete some backups"
    
    # Verify the most recent backup is kept
    most_recent_backup = max(backups, key=lambda x: x.timestamp)
